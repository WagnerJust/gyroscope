...
- [ ] Conformance / `check` command — read-only `gyroscope check [repo]`, the inverse of `init`: does this repo still conform to the standard gyroscope would produce? Exit 0 conformant / non-zero on drift, for CI. Verifies (gyroscope's own output only — NOT maturity; plumbline owns scoring, "coordinate, don't duplicate"): hub present; hub routes == enabled spokes == files on disk (no dangling routes / orphan spokes); pointer files still the single routing line; SessionStart hook installed and cats the enabled files (`hookPathsFor`); no unfilled `{{...}}` left. `.local/*` spokes are gitignored → their absence in a clone is not a failure, but their route/hook entry (when enabled) must be present. Reuse `Plan`/`renderRoutes`/`hookPathsFor`/`target.All` — it's the inverse of init, not a reimplementation. YAGNI: no `--fix`, no scoring. Automates what dogfood pass 2 did by hand.
